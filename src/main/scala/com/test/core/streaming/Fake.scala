          ...
          .foreach(line =>
            (
              if (hdfs.exists(path) == true) { //проверяем, существует ли уже файл, в который собираемся писать
                val fsDataOutputStream = hdfs.append(path) //создаём исходящий поток, добавляющий записи к существующему файлу
                val outputStreamWriter = new OutputStreamWriter(
                  fsDataOutputStream) //создаём то, что будет писать поток
              ...
              } else { //здесь всё то же самое, только сначала файл создаётся
                val fsDataOutputStream = hdfs.create(path)
